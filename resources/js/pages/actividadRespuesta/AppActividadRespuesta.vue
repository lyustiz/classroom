        ...
        <v-col class="text-center">
            ...
            <v-menu content-class="rounded-xl mx-auto" class="mx-auto" offset-x :close-on-content-click="false" v-model="menu" >
                ...
                <template v-slot:activator="{ on }">
                    <v-btn fab color="primary" small v-on="on" :loading="loading">
...
import actividadRespuestaForm  from './actividadRespuestaForm';
export default {

    mixins:     [ DataHelper],

    components: { 
        'actividad-respuesta-form': actividadRespuestaForm 
    },
    ...
    methods:
    {
        list()
        {
           this.getResource(`actividadRespuesta/pregunta/${this.pregunta.id}`).then( data => this.respuestas = data)
        },
        ...
        {            
            ...
            this.form.id_actividad_pregunta  = this.pregunta.id
            this.storeResource('actividadRespuesta', this.form).then( data => {
        ...
        {
            this.deleteResource(`actividadRespuesta/${respuesta.id}`).then( data =>{
                this.showMessage(data.msj)
                this.list()
            })
        },
        ...
        {
            this.updateResource(`actividadRespuesta/${respuesta.id}/nombre`, respuesta ).then( data =>{
                this.respuestas = data.respuestas
            })
        },
        ...
        {
           let form = { bo_correcta, id_actividad_pregunta:  respuesta.id_actividad_pregunta, id_usuario: this.idUser }
           this.updateResource(`actividadRespuesta/${respuesta.id}/correcta`,  form).then( data =>{
                this.respuestas = data.respuestas
            }) 
        },
